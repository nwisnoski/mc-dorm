  ...
  for(yes.dormancy in c(T,F)){
      ...
      for(t in 1:(tsteps / dt - 1)){
        ...
        if(env.type == "synchronous"){
          env[,1] <- env[,1] * (sin(2*pi*t/period) + 1)/2
        }
...
d.list <- list(0, 1e-4, 1e-3, 1e-2, 0.1, 0.5, 1)
#d.list <- list(0)
...
for(i in 1:nsims){
  ...
  dist.freq <- 0.01 # freq of a disturbance
  dist.extent <- 2/M # what fraction of patches disturbed
  
  out <- lapply(d.list, FUN = run_sim, env.type = "static", disturbance = T,
                period = 100, dist.freq, dist.extent)
