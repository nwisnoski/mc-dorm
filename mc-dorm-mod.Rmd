...
tsteps <- 100      # Number of time steps in model
...
for(t in 1:(tsteps / dt - 1)){
  ...
  N.t1 <- N.t0 + disperse(d = d, N = N.t0, M = M)
...
diversity.dynamics <- data.frame(time = seq(1, tsteps), mean_alpha = rowMeans(richness.dynamics), gamma = gamma.dynamics)


diversity.dynamics %>% gather(mean_alpha, gamma, key = "scale", value = "diversity") %>% 
...
### plot model output
```{r}
# z1 <- apply(results.N, 3L, c)
# z2 <- expand.grid(dimnames(results.N)[1:2])
# data.frame(z2, z1)
# results.df <- as.data.frame.table(results.N)
# colnames(results.df) <- c("species", "site", "time", "abundance")
# 
