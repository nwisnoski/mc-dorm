...
run_sim <- function(d){
  ...
  alpha <- mean(specnumber(t(results.N[,,tsteps])))
  gamma <- sum(rowSums(results.N[,,t]) > 0)
  beta <- gamma / alpha
  return(cbind(dispersal = d, alpha = alpha, beta = beta, gamma = gamma))
...
for(i in 1:nsims){
  ...
  d.list <- list(0, 0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1, 0.5, 0.75, 1)
...
# out <- lapply(d.list, FUN = run_sim)
# out <- matrix(unlist(out), ncol = 4, byrow = T)
colnames(running.out) <- c("dispersal", "alpha", "beta", "gamma")
running.out %>% as_tibble() %>% 
  gather(alpha, beta, gamma, key = "scale", value = "diversity") %>%
  group_by(dispersal, scale) %>% 
  mutate(mean_div = mean(diversity), se = sd(diversity)/n()) %>% 
  ggplot(aes(x = dispersal, y = mean_div, color = scale)) + 
