...
E.j <- function(t, stoch = F){
    0.5*env.ampl*(
      sin((1:M)*2*pi/M + (2*pi/env.period)*t) + 1) + ifelse(stoch, abs(rnorm(1, sd = .1)), 0)
...
run.sim <- function(a){
  ...
  for(t.step in 1:length(timesteps)){
    ...
    # Get environments across patches
    e.vec <- E.j(t.step * dt)
    
    # calculate consumption based on env and species optima
    consumption <- sapply(env.optima, c.ij, E.j = e.vec)
    
    # Step through dynamics
    dispersers <- migrate(N)
    Nt1 <- N + dt*N*(e.ij * consumption * R - m.ij) + dt*dispersers - dt*N*a
...
l.j <- 13.38 # resource loss

e.ij <- 0.2 # conversion efficiencies
m.ij <- 0.25 # mortality rates

a <- .75 # dispersal rate
ext <- .1 # extinction threshold
...
for(disp in disp.vec){
  print(paste("Running dispersal rate",disp))
  mc.dynamics <- run.sim(a = disp)
  mc.div <- div.part(mc.dynamics[[5000]])
